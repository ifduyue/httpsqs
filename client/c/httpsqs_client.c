...
#define RemoteIp "xxx.xxx.xxx.xxx"//处理端IP
...
{ 
    ...
    while(1)
    {
        ...
        for(m = 0; m < loop; m++)
        {
		    ...
		    strcpy(res, strstr(str, pattern));
		    memset(str, 0, strlen(str));
		    strcpy(str, getresult(res));
		    ...
		    if(strchr(str, 'H') == NULL)
		    {
			    ...
			    post_sock = htconnect(RemoteIp, RemotePort);
			    if (post_sock < 0) return;

			    len = strlen(str) + 5;
			    htsend(post_sock, "POST /index.php?m=data&a=update HTTP/1.1\r\n", 10);//修改处理端文件
			    htsend(post_sock, "Content-type: application/x-www-form-urlencoded\r\n", 10);
			    htsend(post_sock, "Host: www.CHANGETOYUREDOMAIN.com\r\n", 10);//您自己的域名
			    htsend(post_sock, "Content-Length: %d\r\n", len, 10);
			    htsend(post_sock, "Connection: close\r\n", 10);
			    htsend(post_sock, "\r\n", 10);
			    htsend(post_sock, "data=%s", str, 10);

			    memset(str, 0, strlen(str));
			    i = 0;
			    while (read(post_sock, data, 1)>0)
			    {
                    if(data[0] == '|')
                    {
			    ...
			    if(fp >= 0)
			    {
				    if(strlen(str))
				    {
					    strcpy(res_2, strstr(str, pattern_2));
					    memset(str, 0, strlen(str));
					    strcpy(str, res_2);
